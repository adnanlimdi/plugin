/* global slikBgColors, slikColor, jQuery, wp, _ */
/**
...
( function() {
	...
	wp.customize.bind( 'ready', function() {
		...
		wp.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( slikBgColors ).forEach( function( context ) {
					var backgroundColorValue;
					if ( slikBgColors[ context ].color ) {
						backgroundColorValue = slikBgColors[ context ].color;
					} else {
						backgroundColorValue = wp.customize( slikBgColors[ context ].setting ).get();
					}
					slikSetAccessibleColorsValue( context, backgroundColorValue, to );
				} );
			} );
		} );
		...
		Object.keys( slikBgColors ).forEach( function( context ) {
			wp.customize( slikBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					// Update the value for our accessible colors for this area.
					slikSetAccessibleColorsValue( context, to, wp.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );

		// Show or hide retina_logo setting on the first load.
		slikSetRetineLogoVisibility( !! wp.customize( 'custom_logo' )() );

		// Add a listener for custom_logo changes.
		wp.customize( 'custom_logo', function( value ) {
			value.bind( function( to ) {
				// Show or hide retina_logo setting on changing custom_logo.
				slikSetRetineLogoVisibility( !! to );
			} );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function slikSetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		var value, colors;
		...
		colors = slikColor( backgroundColor, accentHue );
	 ...
	 * @return {void}
	 */
	function slikSetRetineLogoVisibility( visible ) {
