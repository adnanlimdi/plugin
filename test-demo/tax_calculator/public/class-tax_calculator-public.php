 ...
 do_action( 'litespeed_purge_all' );
	...
	public function tc_select_dropdown(){
		// create curl resource
			...
			if(!empty($doorno)){
  		//call api
				$response = wp_remote_get( 'https://api.counciltaxfinder.com/counciltaxfinder/counciltax/'.$postcode.'?door='.$doorno.'&page=1&userid='.$user_id.'&apikey='.$council_tax_api_key.'' );
				$result = json_decode($response['body']);
				$body[] = json_decode($response['body']);
				$apicall = "Api Call = 1";
				txt_calcy_log("".$userip.' '.$postcode.' '.$doorno.' '.date('d-M-Y h:i:s').' '.$apicall." ");
		...
		<select class="property-address" id='color' style="width:80%;">
			<?php
					foreach ($body as  $resultscalcy) {
						...
						?>
							<option selected data-tax="<?php echo $Tax; ?>" data-band="<?php echo $Band; ?>" value='<?php echo $address; ?>'>
	...
	public function tc_forth_step_html(){
        ...
        $actualsaving 		= $ptax - $b3 ;
        // $actualsaving 		= ($ptax + $stampduty) - $b3 ;
        $rentersaving 		= $ptax - $b3 ;
	...
	<div class="user_selections" >
		<img src="<?php echo  plugin_dir_url( __FILE__ ).'/images/fslogosmall.png' ?>">
			<div class="user_selections_list">
				...
				<p class="c_tax">Council Tax Band <b id="c_band"> <?php echo $band; ?></b></p>
				<?php } ?>
				<p class="c_value">Estimated Value <b id="c_price">£<?php echo number_format($propertyvalue); ?></b>&nbsp;

<span class="tooltipinfo" style="display:none"><i class="fa fa-info-circle"></i>
...
<a href="JavaScript:void(0)" class="back-3-steps b_start"><i class="menu-icon fa fa-arrow-left"></i> Calculate again</a>
	...
	<?php if($actualsaving < 0){ ?>
      <p><strong>GREAT NEWS!</strong> With Fairer Share you will no longer pay Council Tax. Instead, your landlord will pay <span class="text-fairlightblue-color">Proportional Property Tax</span> - it’s only fair that property taxes are paid by those who benefit from property ownership.</p>
    <?php }else{ ?>
	<p><strong>GREAT NEWS!</strong> With Fairer Share you will no longer pay Council Tax. Instead, your landlord will pay <span class="text-fairlightblue-color">Proportional Property Tax</span> - it’s only fair that property taxes are paid by those who benefit from property ownership.</p>
	<?php } ?>
    </div>
	...
	<p>
	We have <strong>capped</strong> any increase in Council Tax to £1,200. Please see below for how much you would pay in Proportional Property Tax (PPT).
	</p>
...
<br>
    ...
    <?php if($ptype =='renter' ){ ?>
        <span>Council Tax Saving</span>
            ...
            <span class="tooltipinfo"><i class="fa fa-info-circle"></i><span class="tooltiptext">You will no longer have to pay Council Tax.</span></span>
        <?php }else{ ?>
...
<!-- <div style="display:none">
			...
			<span class="tooltipinfo"><i class="fa fa-info-circle"></i>
              ...
              <?php }else{ ?>
            <span class="tooltipinfo"><i class="fa fa-info-circle"></i>
             <span class="tooltiptext">With Fairer Share you'll never pay Stamp Duty when you buy your home. Our model assumes you'd move to a property with the same value as your current residence and we spread this saving over 20 years — the average period of time between house sales.</span></span>
			<?php } ?>
			</span> <br><br>
</div> -->
...
Your property value of <span class="text-fairlightblue-color">£<?php echo number_format($propertyvalue); ?></span> and the <strong>0.48%</strong> rate would have given a
PPT of <b>£<?php echo number_format($b3); ?></b>. However, to ease this burden we have capped any increase in property tax to £1,200.
Therefore if we transition to PPT you would pay your current Council Tax of <b>£<?php echo number_format($ptax); ?></b> plus <strong>£1,200</strong>. And when you next move home you will not have to pay Stamp Duty
</p>
...
<span class="tooltipinfo"><i class="fa fa-info-circle"></i><span class="tooltiptext">This is 0.48% of the value of your property</span></span>
...
The property value of <span class="text-fairlightblue-color">£<?php echo number_format($propertyvalue); ?></span> and the <strong>0.48%</strong> rate would have given a
PPT of <b>£<?php echo number_format($b3); ?></b>. However, to ease this burden we have capped any increase in property tax to £1,200.
Therefore if we transition to PPT your landlord would pay your current Council Tax of <b>£<?php echo number_format($ptax); ?></b> plus <strong>£1,200</strong>. And if you decide to buy your own home you will not have to pay Stamp Duty.
</p>
...
<span class="text-fairlightblue-color">£<?php echo number_format($b3); ?></span>
<span class="tooltipinfo"><i class="fa fa-info-circle"></i><span class="tooltiptext">The landlord will <?php if($rentersaving > 0){ ?> pay less <?php }else{ ?> probably pay more <?php } ?> than the current cost of Council Tax. Whilst you won't be paying Council Tax under the new system, some of the Proportional Property Tax may be passed on to you. And you'll never have to pay Stamp Duty if you decide to buy your own home!</span></span>
...
<div class="divider" style="height:2em"></div>
<a href="https://fairershare.org.uk/petition/" target="_blank" class="btn btn-lg btn-fairred btn-circle btn-flat btn-icon-left">Sign the Petition Today</a>
<a href="https://fairershare.org.uk/email-your-mp/?SearchText=<?php echo $postcode; ?>" target="_blank" class="btn btn-lg btn-fairblue btn-circle btn-flat btn-icon-left"><i class="menu-icon fa fa-envelope " style="color:#fff"></i> Email my MP Now</a>
...
function txt_calcy_log($log_msg)
{
    ...
    $log_file_data = $log_filename.'/txtlog_' . date('d-M-Y') . '.log';
